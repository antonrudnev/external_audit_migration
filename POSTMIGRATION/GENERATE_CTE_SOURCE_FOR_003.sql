...
SELECT CONCAT('SELECT ', CONV.EXTERNAL_AUDIT_ID, ' AS EXTERNAL_AUDIT_ID, '''
	,CONV.CONTRACT_NUMBER, ''' AS CONTRACT_NUMBER, '
...
LEFT JOIN LMS ON LMS.CONTRACT_NUMBER = CONV.CONTRACT_NUMBER
	...
	AND (CONV.CNT = 1 OR (CONV.CNT = 2 AND LEFT(LMS.AUDIT_TYPE, 4) = CASE CONV.RN WHEN 1 THEN 'PROJ' WHEN 2 THEN 'EXEC' END))
